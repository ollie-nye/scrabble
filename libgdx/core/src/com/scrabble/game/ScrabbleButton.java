...
public class ScrabbleButton extends ButtonBase {
	...
	public ScrabbleButton(String text, ScrabbleButtonStyle style, int xCoor, int yCoor, int boardOrPlayer) {
		...
		score = new Label(" ", new LabelStyle(style.font, style.fontColor));
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		if (boardOrPlayer == 0) {
			...
			if (Board.getInstance().getTile(loc) == null) {
				...
				score.setText(Integer.toString((Board.getInstance().getTile(loc).getScore())));
			}
		...
		if (isPressed() && isPressed == false) {
			...
			if (boardOrPlayer > 0 && boardOrPlayer - 1 == Scrabble.currentPlayer) {
				Board.getInstance()
						.partialPlace(PlayersContainer.getInstance().getPlayer(boardOrPlayer - 1).getLetter(xCoor));
			}
