...
public abstract class Move implements Serializable {
	...
	private int calculateConnectedTileScores(Coordinate coordinate) {
		...
		if (!playedTiles.containsKey(board.getTile(tempCoordinate)) && tempCoordinate.getY() >= 0
				&& board.getTile(tempCoordinate) != null) {
			tempScore += board.getTile(tempCoordinate).getScore();
		}

		tempCoordinate = coordinate.getNear('D');

		if (tempCoordinate.getY() <= 14) {
			if (!playedTiles.containsKey(board.getTile(tempCoordinate)) && board.getTile(tempCoordinate) != null) {
				tempScore += board.getTile(tempCoordinate).getScore();
			}
		}
		tempCoordinate = coordinate.getNear('L');

		if (!playedTiles.containsKey(board.getTile(tempCoordinate)) && tempCoordinate.getX() >= 0
				&& board.getTile(tempCoordinate) != null) {
			tempScore += board.getTile(tempCoordinate).getScore();
		}
